...
function renderBracket(numTeams) {
  ...
  else if (totalPlayers <= 8) baseHeight = 50;    // 3 vòng: Vòng 1/8, Tứ kết/Bán kết, Chung kết
  else if (totalPlayers <= 16) baseHeight = 60;   // 4 vòng
  else baseHeight = 65;                           // 5 vòng trở lên
  ...
  for (let r = 0; r < roundCount; ++r) {
    ...
    for (let i = 0; i < matches.length; ++i) {
      ...
      // Tính vị trí top: mỗi ô trận bắt đầu ở hàng rowStart và có chiều cao
      // rowSpan * baseHeight. Đối với knock‑out bracket, mỗi cấp sâu hơn
      // sẽ có chiều cao gấp đôi cấp trước để tạo khoảng cách.
  ...
  containerHtml += '</div>';
  document.getElementById('koBracket').innerHTML = headerHtml + containerHtml;
}
